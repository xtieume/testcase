...
only: process groups (`os.killpg`), `sh -c` and git — not for Windows. Nor is GNU coreutils
the environment: `timeout` does not exist on macOS and `sed -i` takes an argument there, so
a row using either runs on Linux and fails everywhere else. A check never needs `timeout`
anyway — `--timeout N` already bounds every check, and one that hits it is `FAIL`, detected
structurally rather than by the words in its output. A ledger is an executable file: read
every row before running it, as you would a `Makefile`. A break that touches state outside
the repo (databases, services, `$HOME`) is not undone.
